...
**Phase: v0.2.3 — live-bug fixes.** UPI capture un-broken (path mismatch `cache/` vs `code_cache/`), now real-time (30s drain); feature deletes sync via `deleted_features` tombstones (schema v11); app lock fixed (`FlutterFragmentActivity`); Profile "Source" not "Open source". See changelog.
