...
**Phase: v0.2.1 — shipped.** Transactions edit/delete + tombstone sync, UX audit, arm64 release APK live on the v0.2.1 GitHub release. Android-only (Windows scaffold removed for good — single platform). Gradle page-aligned native libs (`useLegacyPackaging=false`) — no "package invalid". CI removed (on-demand build + release). UX pass: onboarding 4-step flow + live permission status + 2s poll, profile edit-name, reports pie total/trend guard, categories segmented tabs + vector icons, terms consent+date, export→Downloads, app lock PIN-only. **Full backup sync** — budgets/wallets/recurring/objectives/debts + custom categories sync to Supabase (schema v9→v10, migrations 0002-0004), auto-backup timer flushes every 30s. Server `categories` seed (0003) fixes budget push FK 23503. **Capture everywhere** — any app notification with ₹ amount captured (bank/messaging incl.), money-in = income txn, parser keyword-gated (schema v10). `flutter analyze` clean, parser tests pass. Full `flutter test` still blocked on this Windows host (sqlite3 native-assets, `docs/troubleshooting.md`). Next: on-device verify.
