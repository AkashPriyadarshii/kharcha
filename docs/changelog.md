...
- **Stack swap complete.** Flutter/Dart/Supabase deleted: `lib/`, `test/`, `android/` (incl. KMP `parser-core`), `supabase/`, `pubspec.*`. Repo = `android-app/` (Kotlin Compose) + `kharcha-core/` (Rust) only. Fully offline, zero Supabase, zero AI.
- **Deterministic capture in Rust.** `kharcha-core` v0.1: single parser engine (generic UPI + HDFC/SBI/ICICI-style bank formats), `parse_capture(s)`, paise-i64 amounts, triple-signal dedupe (ref + 5-min window + content hash), batch API, spam/recharge/OTP non-transaction filter, bill split, filter/all-in-one ops. 59/59 tests (24 unit + 35 parity).
