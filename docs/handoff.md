...
- **2026-08-06** — **CI + Windows test blocker:** added `.github/workflows/ci.yml` (analyze + test on ubuntu-latest) as the reliable test gate. Local `flutter test` on Windows crashes on sqlite3 native-assets (docs/troubleshooting.md). Code for 2.1–2.3 on branch `feat/manual-entry`, analyze clean. (Akash)
